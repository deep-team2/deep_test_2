...
isna_count_dict = isna_count[isna_count > 0].to_dict()
print(isna_count_dict)
print(type(df["진동"].iloc[0]).__name__)
value_count = df["판정"].value_counts().to_dict()
print(value_count)
...
df["온도"] = df["온도"].fillna(temp_avg)
df["압력"] = df["압력"].fillna(press_med)
df["진동"] = df["진동"].fillna(vib_avg)
